...
from algorithms_2d import nfdh, ffdh, best_fit_2d, brute_force_2d
...
class Packing2DApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        ttk.Label(rect_frame, text="Width:").grid(row=0, column=0, sticky="w")
        self.rect_width_entry = ttk.Entry(rect_frame)
        self.rect_width_entry.grid(row=0, column=1, pady=5)
        
        ttk.Label(rect_frame, text="Height:").grid(row=1, column=0, sticky="w")
        self.rect_height_entry = ttk.Entry(rect_frame)
        self.rect_height_entry.grid(row=1, column=1, pady=5)
        
        ttk.Button(rect_frame, text="Add Rectangle", command=self.add_rectangle, style="Primary.TButton").grid(row=2, column=0, columnspan=2, pady=5, sticky="we")
    ...
    def add_rectangle(self):
        try:
            w = int(self.rect_width_entry.get())
            h = int(self.rect_height_entry.get())
    ...
    def update_rect_list(self):
        ...
        for i, (w, h) in enumerate(self.rectangles, 1):
    ...
    def run_algorithm(self, algorithm_name):
        try:
            ...
            if algorithm_name == "NFDH":
                ...
                shelves = best_fit_2d(self.rectangles, container_w, container_h)
            ...
            used_area = sum(rect[2]*rect[3] for rect in packed_rects)
